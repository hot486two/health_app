...
public class DBHelper {
    ...
    public long insertActivityReport(int type, long time, int year, int month, int day, int hour, int[] dataArray, String subData) throws SQLiteConstraintException {
        ...
        ContentValues insertValues = new ContentValues();					//import android.content.ContentValues;등록한 데이터를 기록하는 ContentValues 객체 생성
        ...
        synchronized (mDb) {			//스레드 동기화
            if(mDb == null)
                return -1;
            return mDb.insertOrThrow(TABLE_NAME_ACCEL_REPORT, null, insertValues);//mDB = import android.database.sqlite.SQLiteDatabase's Object.-> insertOrThrow
            //I use insertOrThrow instead of insert because of SQLException
        }
    }
