...
public class ServiceMonitoring {
    ...
    public static void startMonitoring(Context context) {
        AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);//system level service control./A AlarmManager for receiving intents at the time of your choosing.
        Intent intent = new Intent(context, ServiceMonitoringBR.class);
        PendingIntent pi = PendingIntent.getBroadcast(context, 0, intent, 0);			//Retrieve a PendingIntent that will perform a broadcast
        am.setRepeating(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime(), SERVICE_RESTART_INTERVAL, pi);//repeating alarm.주기를 정하고 반복적으로 동작
    }
